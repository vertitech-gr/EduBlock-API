/**
 * Before resolving an entity, ensures that `edublockAPI` is initialized, throws an error otherwise.
 * @template T
 * @param {T} resolverClass
 * @param {function(T): edublockAPI} pickedublockAPI
 * @returns {T}
 */
export const withInitializededublockAPI = (
  resolverClass,
  pickedublockAPI = ({ dock }) => dock,
) => class extends resolverClass {
  async resolve(url) {
    if (pickedublockAPI(this).isInitialized()) {
      return super.resolve(url);
    } else {
      throw new Error('edublockAPI is not connected');
    }
  }
};
